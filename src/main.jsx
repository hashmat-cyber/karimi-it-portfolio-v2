...
function Resume({t}){return <><PageHero num="08" title={t.resumeTitle} sub={t.resumeSub} t={t}/><section className="section"><div className="container resumeGrid"><GlassCard className="resumeMain"><div className="resumeHeader"><div><span className="eyebrowSmall">{t.profileLabel}</span><h2>Sayed Hashmat Karimi</h2><p>{t.role}</p></div><div className="resumeCode">SK<br/><span>IT</span></div></div><div className="resumeRows"><div><GraduationCap/><span>{t.resumeEducationLabel}</span><b>{t.resumeEducationValue}</b></div><div><Code2/><span>{t.resumeTechFocusLabel}</span><b>{t.resumeTechFocusValue}</b></div><div><Award/><span>{t.resumeCredentialsLabel}</span><b>{t.resumeCredentialsValue}</b></div><div><Layers3/><span>{t.resumeProjectsLabel}</span><b>{t.resumeProjectsValue}</b></div></div><button className="btn btnPrimary resumeBtn" onClick={() => window.open("/assets/documents/Sayed-Hashmat-Karimi-CV.pdf", "_blank")}><Download/>{t.downloadResumeLabel}</button></GlassCard><GlassCard className="resumeSide"><span className="eyebrowSmall">{t.coreSignalsLabel}</span><div className="signal"><strong>90%</strong><span>{t.overallSchoolResultLabel}</span></div><div className="signal"><strong>03</strong><span>{t.featuredProjectsLabel}</span></div><div className="signal"><strong>04+</strong><span>{t.technicalCredentialsLabel}</span></div></GlassCard></div></section></>}
